...
define([
    ...
    var Quiz = {
        ...
        handleSubmissionResponse: function(response) {
            ...
            if (response.success) {
                ...
                if (question.rationale) {
                    var ratStyle = 'background: #e8f4f8; border-left: 4px solid #17a2b8;';
                    html += '<div class="rationale-section mb-3 p-3 rounded" style="' + ratStyle + '">';
                ...
                if (question.difficulty) {
                    ...
                    var diffLabel = question.difficulty.charAt(0).toUpperCase() + question.difficulty.slice(1);
                    html += '<div class="col-md-4 mb-2">';
                    html += '<strong>Difficulty:</strong> ';
                    html += '<span class="badge badge-' + diffColor + '">' + diffLabel + '</span>';
                    html += '</div>';
                }

                // Bloom's Level
                if (question.bloomlevel) {
                    var bloomLabel = question.bloomlevel.charAt(0).toUpperCase() + question.bloomlevel.slice(1);
                    html += '<div class="col-md-4 mb-2">';
                    html += "<strong>Bloom's Level:</strong> ";
                    html += '<span class="badge badge-info">' + bloomLabel + '</span>';
                    html += '</div>';
                }
        ...
        renderTrustworthinessBadge: function(question) {
            ...
            var badgeStyle = 'background: ' + color + '; color: white; padding: 4px 10px;';
            var divStyle = 'border-left: 4px solid ' + color + '; padding: 10px 15px;';
            divStyle += ' background: #f8f9fa; border-radius: 4px;';

            var html = '<div class="trustworthiness-badge mb-3" style="' + divStyle + '">';
            html += '<div class="d-flex align-items-center">';
            html += '<span class="badge mr-2" style="' + badgeStyle + '">' + score + '%</span>';
        ...
        renderAnsweredState: function(question, questionId) {
            ...
            // Rationale
            if (question.rationale) {
                var ratStyle2 = 'background: #e8f4f8; border-left: 4px solid #17a2b8;';
                html += '<div class="rationale-section mb-3 p-3 rounded" style="' + ratStyle2 + '">';
                html += '<h5><i class="fa fa-lightbulb-o text-info mr-2"></i>Explanation</h5>';
                html += '<p class="mb-0">' + question.rationale + '</p>';
                html += '</div>';
            }
