...
class kimi_provider extends provider_base {
    protected $name = 'kimi';
    protected $description = 'Kimi AI Global (Moonshot) Provider';
    ...
    public function generate_questions($input, array $options = []): array {
        ...
        foreach ($models as $model) {
            ...
            try {
                ...
                $url = rtrim($this->config['baseURL'] ?? 'https://api.moonshot.ai/v1', '/');
                ...
                if (!$textresponse) {
                    throw new \Exception('Kimi response missing content');
                }
        ...
        throw new \Exception('Kimi generation failed: ' . ($lasterror ? $lasterror->getMessage() : 'unknown error'));
    }
